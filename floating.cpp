...
glVertex2i(x22,y22);
glVertex2i(x11,y22);
glEnd();glColor3f(0.0,0.0,0.0); //lower pipe outline
...
int darw_box(int xd1,int xd2,int yd1,int yd2) //to draw boxes in menu
{
...
int draw_selbox(int xd1,int xd2,int yd1,int yd2) //to draw sub boxes in menu
{
...
glColor3f(0.0,0.50,1.0);
output(110,270,"score=");
// to display a box with high scores
char buff[10];
sprintf(buff,"%d",points[games-1]); // to covert an integer to character
output(180,270,buff);
}
int bck() //back box in submenus
{
darw_box(50,150,350,400);
glColor3f(0.0,0.50,1.0);
output(70,375,"back");
}
int functions() //controls part
{
bg();
bck();
glColor3f(0.0,0.0,1.0);
...
int info() // display part about the cerators
{
bg();
bck();
glColor3f(0.0,0.0,1.0);
...
{
flag=0;
//goes back to the main menu from sub menus
flag1=0;
glutPostRedisplay();
}
}
...
glutIdleFunc(move_rect_n_ball);glutKeyboardFunc(keystrk);
glutMouseFunc(mouse);
